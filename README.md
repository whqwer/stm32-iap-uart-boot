...
```c
typedef struct {
    uint16_t page_count;       // 页数计数 (升级时使用，平时为0)
    uint32_t firmware_CRC;     // 固件 CRC32 校验值
    uint32_t version;          // 固件版本号 (升级时保留)
} ImageConfig_t;  // 总共10字节
```

**字段说明：**
- `page_count`: 升级过程中记录已写入页数，升级完成后清零
- `firmware_CRC`: 运行区固件的 CRC32 校验值，用于启动验证
- `version`: 固件版本号，升级时自动保留，APP可用于判断升级是否成功
...
```
1. 初始化更新
   ├─ 设置 g_expected_page_count (从配置读取)
   ├─ 设置目标地址: 0x08008000 (升级区)
   └─ 进入更新模式 (UART1_in_update_mode = 1)

2. 擦除升级区
   ├─ Erase_Image(0)  // 0=升级区
   ├─ 擦除 48KB (6个扇区)
   └─ 失败则清除 page_count 并退出
...
4. 接收完成判断
   ├─ 检查 page_index >= expected_page_count - 1
   ├─ 是 → 继续下一步
   └─ 否 → 继续接收

5. 计算升级区 CRC
   ├─ Calculate_Image_CRC(UPDATE_REGION_BASE, total_size)
   └─ 保存升级区 CRC 值

6. 复制升级区到运行区
   ├─ Copy_Update_To_Runapp(page_count)
   ├─ 先擦除运行区 (48KB)
   ├─ 逐页复制数据
   └─ 复制失败 → 清除 page_count 并返回错误

7. 验证运行区 CRC
   ├─ Calculate_Image_CRC(RUNAPP_REGION_BASE, total_size)
   ├─ 比较与升级区 CRC 是否一致
   │  ├─ CRC一致 → 升级成功 ✓
   │  └─ CRC不一致 → 升级失败 ✗
   └─ 失败则清除 page_count 并返回错误

8. 更新配置区
   ├─ 保存运行区 CRC (g_config.firmware_CRC = run_crc)
   ├─ 清除升级标志 (g_config.page_count = 0)
   ├─ ⚠️ 版本号自动保留 (Config_Write 自动处理)
   └─ 写入配置区 Flash
...
└────────┬───────────────────────┘
    ...
    重新上电后
         ↓
┌────────────────────────────────┐
│ Bootloader 检测到 page_count>0  │
├────────────────────────────────┤
│ → 进入更新模式等待固件          │
│ → 重新接收固件数据              │
│ → 版本号在Config_Write时自动保留│
└────────────────────────────────┘
   结果：设备仍可正常运行 ✓
...
3. 修改 `page_count` 字段（偏移 +0 字节）
   - 设为 `0x0000` → 正常运行模式（2字节小端序）
4. 写回配置区
5. 重启设备
...
```bash
# 使用 OpenOCD
flash erase_sector 0 3 3  # 擦除第3扇区（配置区）
```

**注意**：擦除后下次启动会自动检测运行区固件并初始化配置。
...
#### 配置和升级管理函数（iap_image.c）

| 函数 | 功能 |
|------|------|
| `Config_Read()` | 从 Flash 配置区读取 ImageConfig_t |
| `Config_Write()` | 写入配置区（含擦除）<br>**⚠️ 自动保留版本号** |
| `Config_Init()` | 初始化配置（自动检测运行区固件） |
| `Calculate_Image_CRC()` | 计算指定区域的 CRC32 |
| `Erase_Image()` | 擦除指定区域（0=升级区, 1=运行区） |

**版本号相关函数（已注释）：**
```c
// int8_t Config_Set_Version(uint32_t new_version);  // 设置版本号
// uint32_t Config_Get_Version(void);                 // 读取版本号
```
注：版本号功能已在代码中实现但被注释，需要时可取消注释使用。
...
- **断电保护** - `page_count` 标志确保更新中断电可恢复
- **版本号保留** - `Config_Write()` 自动保留版本号，升级时不丢失
...
| 断电后可能无法恢复 | ✅ page_count 标志保护 |
| 版本号升级后丢失 | ✅ Config_Write 自动保留版本号 |
| 需要手动恢复 | ✅ 完全自动化故障恢复 |
